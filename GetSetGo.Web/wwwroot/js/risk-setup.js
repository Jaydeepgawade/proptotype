(() => {
  ...
  function update(showValidation = interacted) {
    ...
    form.querySelectorAll('[data-style-card]').forEach(card => {
      ...
      error.textContent = showValidation ? message : ''; card.classList.toggle('has-error', showValidation && !!message); status.textContent = capital > 0 ? 'Active' : 'Disabled'; status.classList.toggle('active', capital > 0);
    ...
    const left = total - sum; allocated.textContent = money(sum); remaining.textContent = money(left); state.classList.toggle('over', showValidation && left < 0);
    ...
    valid = valid && sum > 0 && left >= 0;
    formError.textContent = showValidation && !valid ? 'Correct the highlighted values before saving.' : '';
    submit.disabled = showValidation && !valid;
    return valid;
  }
  form.querySelectorAll('input, select').forEach(input => {
    const validateAfterInteraction = () => { interacted = true; update(true); };
    input.addEventListener('input', validateAfterInteraction);
    input.addEventListener('change', validateAfterInteraction);
  });
  form.addEventListener('submit', event => {
    interacted = true;
    if (!update(true)) event.preventDefault();
  });
  update(false);
})();
