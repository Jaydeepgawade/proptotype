    ...
    function load() {
        ...
        request = $.getJSON(container.dataset.url).done(function (response) {
            ...
            try {
                ...
                const levels = [['entry', 'ENTRY', '#0b5cab'], ['stop', 'STOP', '#c43f4b'], ['target', 'TARGET', '#11855b']];
                levels.forEach(([key, title, color]) => candles.createPriceLine({ price: Number(container.dataset[key]), title, color, lineWidth: 2, lineStyle: LightweightCharts.LineStyle.Dashed, axisLabelVisible: true }));
                candles.applyOptions({ autoscaleInfoProvider: original => { const info = original(); if (!info) return info; const prices = levels.map(([key]) => Number(container.dataset[key])); return { ...info, priceRange: { minValue: Math.min(info.priceRange.minValue, ...prices), maxValue: Math.max(info.priceRange.maxValue, ...prices) } }; } });
