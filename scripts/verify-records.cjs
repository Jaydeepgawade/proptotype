...
const original = Array.from({length:27}, (_, i) => {
...
next.fire('click'); assert.equal(body.children.length,7); assert.equal(next.disabled,true);
...
search.value='stock 2'; search.fire('input'); assert.deepEqual(body.children.map(r=>r.cells[0].textContent),['Stock 2','Stock 20','Stock 22','Stock 24','Stock 26']);
reset.fire('click'); headers[1].children[0].fire('click'); assert.equal(body.children[0],original[0]); headers[1].children[0].fire('click'); assert.equal(body.children[0],original[26]);
assert.equal(headers[1].attrs['aria-sort'],'descending');
headers[3].children[0].fire('click'); assert.equal(body.children[0],original[1]);
sizes.value='50'; sizes.fire('change'); assert.equal(body.children.length,27); assert.equal(body.children[26],original[0]);
reset.fire('click'); assert.equal(body.children.length,10); assert.equal(body.children[0],original[0]);
console.log('PASS: pagination, row count, combined search/filter, empty results, numeric/date sorting, null dates, reset, original row identity.');
